...
class SurveyAndDesignForm(forms.Form):
    # Define fields for the "Survey_and_Design" form
    ('surveying_of_the_area_to_determine_road_alignment,_width…','surveying of the area to determine road alignment, width…')
    ('Create detailed road design plans')

    detailed_design = forms.CharField(
...
class MediaFileForm(forms.Form):
    ...
    file = forms.FileField(widget=forms.ClearableFileInput(attrs={'multiple': True}))  # Allow multiple file uploads
...
class StartPointForm(forms.Form):
    chainage = forms.IntegerField(label='Start point chainage (type: integer)')
    location = forms.CharField(label='Share a location (type: map)', widget=LeafletWidget(attrs={'map_width': '100%', 'map_height': '300px'}))
    photo = forms.ImageField(label='Share start point photo (type: image)')


class EndPointForm(forms.Form):
    chainage = forms.IntegerField(label='End point chainage (type: integer)')
    location = forms.CharField(label='Share a location (type: map)', widget=LeafletWidget(attrs={'map_width': '100%', 'map_height': '300px'}))
    photo = forms.ImageField(label='Share end point photo (type: image)')
